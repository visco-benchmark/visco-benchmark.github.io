...
sortTable.number = function (Cell) {
    var tag = sortTable.stripTags(Cell.innerHTML).replace(/[^-\d.]/g, '');
    if (tag === '-' || tag === '') {
        if (Cell.getAttribute("value") === "max") {
    ...
    for (var i = 0; i < Tables.length; i++) {
        ...
        for (var rowNum = 0; rowNum < THead.rows.length; rowNum++) {
            for (var cellNum = 0, colNum = 0; cellNum < THead.rows[rowNum].cells.length; cellNum++) {
                ...
                THead.rows[rowNum].cells[cellNum].setAttribute('data-js-sort-colNum', rowNum === 0 ? colNum : colNum + 3);
                Handler = sortTable.getClickHandler(Tables[i], rowNum === 0 ? colNum : colNum + 3);
